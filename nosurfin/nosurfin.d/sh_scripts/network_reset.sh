...
ipv4_dis () {
    ...
    if [ $ipv4_nat_80 -gt 0 ]
    then
	rule_id=$(iptables -t nat -L OUTPUT --line-numbers | grep "http\s.\+ports\s8080" | awk '{match($0, "[0-9]+")}END{print substr($0, RSTART, RLENGTH)}')
    	iptables -t nat -D OUTPUT $rule_id
    fi

    if [ $ipv4_nat_443 -gt 0 ]
    then
	rule_id=$(iptables -t nat -L OUTPUT --line-numbers | grep "https\s.\+ports\s8080" | awk '{match($0, "[0-9]+")}END{print substr($0, RSTART, RLENGTH)}')
    	iptables -t nat -D OUTPUT $rule_id
    fi
}
ipv6_dis () {
...
if [ $(sysctl -a | grep net.ipv6 | wc -l) -gt 0 ]
then
    ipv6_dis
fi
